...
import { getBatchSummaries, getPlatformSummaries } from "@/lib/evidence-analytics";
...
function PlatformsContent() {
  ...
  return <ClientLayout><ClientPageHeader eyebrow="AI 平台" title="平台表现" description="仅比较本批次实际采集的平台；未采集的平台不会以 0% 参与比较。" batches={batchNames} selectedBatch={selectedBatch} /><section className="dashboard-card"><div className="section-top"><div><p className="eyebrow">平台筛选</p><h2>本批次平台对比</h2></div><label className="inline-select"><span>平台</span><select value={selectedPlatform} onChange={(event) => setPlatform(event.target.value)}><option>全部平台</option>{rows.map((row) => <option key={row.platform}>{row.platform}</option>)}</select></label></div><PlatformPerformanceTable rows={displayedRows} batch={selectedBatch} /></section>{selectedPlatform !== "全部平台" && <section className="dashboard-card"><div className="section-top"><div><p className="eyebrow">跨批次</p><h2>{selectedPlatform} 复查记录</h2></div></div><div className="table-scroll"><table className="data-table"><thead><tr><th>复查批次</th><th>状态</th><th>记录数</th><th>提及率</th><th>前三率</th><th>优势率</th><th>首位率</th><th>平均位置</th></tr></thead><tbody>{crossBatch.map(({ batch, row }) => row ? <tr key={batch}><td>{batch}</td><td>已采集</td><td>{row.recordCount}</td><td>{row.mentionRate?.toFixed(1)}%</td><td>{row.topThreeRate?.toFixed(1)}%</td><td>{row.advantageRate?.toFixed(1)}%</td><td>{row.firstMentionRate?.toFixed(1)}%</td><td>{row.averageMentionPosition?.toFixed(1) ?? "—"}</td></tr> : <tr key={batch}><td>{batch}</td><td colSpan={7} className="missing-cell">本批次未采集</td></tr>)}</tbody></table></div></section>}</ClientLayout>;
}
